...
export default function FoodAndDrink({ sheet }: { sheet: any[][]} ) {
  ...
  const drinkTableRows = makeTable(drinkData, numCols);
  // console.log(drinkTableRows)

  const foodData = selectColumns(sheet, 3, 4);
  const foodTableRows = makeTable(foodData, numCols);

  const cakeData = selectColumns(sheet, 6, 7);
  const cakeTableRows = makeTable(cakeData, 1);

  const donutData = selectColumns(sheet, 9, 10);
  const donutTableRows = makeTable(donutData, numCols);

  return (
    <>
    <Header />
    <main>
      <div className="flex flex-col mx-auto bg-zinc-50 text-zinc-800 p-2 md:p-4 justify-start max-w-2xl w-full gap-8 md:gap-6">
        <div>
          <Link href="/" className="text-slate-700 hover:text-slate-500">
            ← Back to Home
          </Link>
        </div>
        <div className="flex flex-col gap-2">
          <h2 className="text-center">Drinks ☕</h2>
            <table className="table-auto w-3/4 mx-auto md:w-full border-separate border-spacing-y-2 border-spacing-x-8 bg-white shadow-md rounded-md">
              <tbody>
                {drinkTableRows}
              </tbody>
            </table>
        ...
        <div className="flex flex-col gap-2">
          ...
          <table className="table-fixed text-center m-auto w-3/4 border-separate bg-white shadow-md rounded-md">
              <tbody>
                {cakeTableRows}
              </tbody>
            </table>
